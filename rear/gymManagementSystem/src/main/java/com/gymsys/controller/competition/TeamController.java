...
public class TeamController {
    ...
    @PostMapping("/add")
    public Result<?> addTeam(@RequestBody AddTeamDTO addTeamDTO) {
    ...
    @PutMapping("/update")
    public Result<?> updateTeam(@RequestBody UpdateTeamDTO updateTeamDTO) {
        teamService.updateTeam(updateTeamDTO);
        return Result.success(MessageConstant.UPDATE_SUCCESS);
    }

    @GetMapping("/list")
    public Result<IPage<Team>> listCompetition(@Valid ListTeamDTO listTeamDTO) {
        return Result.success(teamService.listTeamDTO(listTeamDTO));
    }
